...
class DECLoss(nn.Module):
    ...
    def forward(self, encoder: torch.nn.Module, batch: torch.Tensor) -> torch.Tensor:
        ...
        # target = target_distribution(output).detach() # computation of P
        target = target_distribution(output) # computation of P
        # dec_loss = self.loss_function(output.log(), target) / output.shape[0]
        dec_loss = self.loss_function(torch.log(output), target) / output.shape[0]
        # dec_loss = self.loss_function(torch.log(output), target)
        # dec_loss = F.kl_div(torch.log(output), target)
        return dec_loss
